...
(ert-deftest eglot-rcpp-consult-symbol-state-accepts-looked-up-xref-items ()
  (let* ((location (xref-make-file-location "/tmp/demo.cpp" 3 5))
         (xref (xref-make "demo" location))
         (opened nil)
         (marker nil)
         (jump-calls nil)
         (state nil))
    (cl-letf (((symbol-function 'consult--temporary-files)
              ...
              ((symbol-function 'consult--jump-state)
               (lambda ()
                 (lambda (action value)
                   (push (list action value) jump-calls))))
              ((symbol-function 'consult--marker-from-line-column)
               (lambda (file line column)
                 (setq marker (list file line column))
                 marker)))
      (setq state (eglot-rcpp--consult-symbol-state))
      (funcall state 'preview xref)
      (should (equal opened "/tmp/demo.cpp"))
      (should (equal marker '("/tmp/demo.cpp" 3 5)))
      (should (equal (car jump-calls)
                     (list 'preview '("/tmp/demo.cpp" 3 5)))))))
