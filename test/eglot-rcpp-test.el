...
(ert-deftest eglot-rcpp-install-hooks-activates-existing-managed-buffers ()
  (eglot-rcpp-test--with-modes
   (let ((eglot-rcpp--hooks-installed nil)
         (activated nil)
         (buffer (generate-new-buffer " *eglot-rcpp-existing*")))
     (cl-letf (((symbol-function 'eglot-rcpp--sync-consult-integration)
                (lambda () nil))
               ((symbol-function 'eglot-rcpp--activate-current-buffer)
                (lambda ()
                  (push major-mode activated))))
       (unwind-protect
           (with-current-buffer buffer
             (eglot-rcpp-test-r-mode)
             (eglot-rcpp--install-hooks)
             (should (member 'eglot-rcpp-test-r-mode activated)))
...
(ert-deftest eglot-rcpp-project-server-buffers-use-current-server-not-managed-mode ()
  (let ((server-a (list :server 'a))
        (server-b (list :server 'b))
        (buf-a (generate-new-buffer " *eglot-rcpp-a*"))
        (buf-b (generate-new-buffer " *eglot-rcpp-b*"))
        (buf-c (generate-new-buffer " *eglot-rcpp-c*")))
    (unwind-protect
        (cl-letf (((symbol-function 'eglot-rcpp--project-buffers)
                   (lambda (&optional _root) (list buf-a buf-b buf-c)))
                  ((symbol-function 'eglot-rcpp--current-server)
                   (lambda ()
                     (pcase (buffer-name (current-buffer))
                       (" *eglot-rcpp-a*" server-a)
                       (" *eglot-rcpp-b*" server-a)
                       (" *eglot-rcpp-c*" server-b)
                       (_ nil)))))
          (should (equal (eglot-rcpp--project-server-buffers "/tmp/pkg")
                         (list buf-a buf-c))))
      (mapc (lambda (buffer)
              (when (buffer-live-p buffer)
                (kill-buffer buffer)))
